...
class Generator(object):
    ...
    def _get_descriptions(self, task, list_of_pairs, **kwargs):
        ...
        descriptions = ""
        lines = set()
        for key_dict, v in list_of_pairs:
            # key_dict now contains overall_score and word_scores
            if isinstance(v, tuple):
                v = v[0]
        
            overall_score = key_dict.get("overall_score", 0.0)
            word_scores = key_dict.get("word_scores", {})
        
            word_score_str = ", ".join([f"{w}: {s:.2f}" for w, s in word_scores.items()])
            descriptions += f"Overall score: {overall_score:.3f}, Word scores: {{{word_score_str}}}"
    ...
    def __call__(self, task_dict: Dict[Text, List[Tuple[Dict[str, any], Text]]], **kwargs):
